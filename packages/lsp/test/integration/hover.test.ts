 ...
 *
 * No ts.LanguageService is available, so all hover
...
function App() {
  ...
  return <div>{count()}</div>;
    ...
    expect(server.hover("/test/app.tsx", 2, 10)).toBeNull();
  });

  it("returns null for nonexistent file", () => {
    expect(server.hover("/test/nope.tsx", 0, 0)).toBeNull();
  });

  it("returns null for empty file", () => {
    server.addFile("/test/empty.tsx", "");
    expect(server.hover("/test/empty.tsx", 0, 0)).toBeNull();
  });

  it("does not crash on out-of-range position", () => {
    server.addFile("/test/edge.tsx", "function App() { return <div />; }");
    expect(server.hover("/test/edge.tsx", 1000, 0)).toBeNull();
  });
});
