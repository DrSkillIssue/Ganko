...
describe("CSS policy rules", () => {
  ...
  describe("css-policy-touch-target", () => {
    it("reports button height below minimum (24px for wcag-aa)", () => {
      const ds = lint(`button { height: 20px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("20px")
      expect(first(hits).message).toContain("24px")
      expect(first(hits).message).toContain("button")
    })

    it("does not report button height at minimum", () => {
      const ds = lint(`button { height: 24px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(0)
    })

    it("reports input min-height below minimum", () => {
      const ds = lint(`input { min-height: 18px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("input")
    })

    it("reports button width below minimum", () => {
      const ds = lint(`button { width: 16px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("width")
    })

    it("reports button horizontal padding below minimum", () => {
      const ds = lint(`button { padding-left: 4px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("padding")
      expect(first(hits).message).toContain("8px")
    })

    it("does not report padding on non-interactive elements", () => {
      const ds = lint(`.text { padding-left: 2px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(0)
    })

    it("detects .btn class as button", () => {
      const ds = lint(`.btn { height: 16px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("button")
    })
    ...
    it("detects select as input", () => {
      const ds = lint(`select { height: 18px; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("input")
    })
    ...
    it("converts rem values for comparison", () => {
      const ds = lint(`button { height: 1rem; }`)
      const hits = ds.filter((d) => d.rule === "css-policy-touch-target")
      expect(hits).toHaveLength(1)
      expect(first(hits).message).toContain("16px")
    })

    it("does not flag visually hidden inputs (position: absolute + opacity: 0)", () => {
      const ds = lint(`
