...
export type RuleId =
  ...
  | "jsx-layout-policy-touch-target"
...
export const RULES: readonly RuleEntry[] = [
  ...
  },
  {
    ...
    "messages": {
      ...
      "signalIndexedByLoop": "Signal '{{name}}' is indexed by a loop-dependent key inside <{{component}}>. Each item now depends on the entire signal value, so updating one key re-runs all items. Use createStore for keyed state or derive a per-item accessor outside the loop.",
