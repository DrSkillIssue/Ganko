...
**AGENTS.md note:** `const enum` declarations use `as` syntax internally in TypeScript's emit, but the `as Type` prohibition in AGENTS.md applies to user-written type assertions, not compiler-generated enum patterns. No `as` casts appear in the source.
...
- `cascade-builder.ts:57,74,297-307` — `collectMonitoredDeclarations`, `buildCascadeMapForElement`
...
- `graph.ts:17` — `LayoutCascadedDeclaration.source`
- `cascade-builder.ts:306,349-358` — inline style source assignment, `doesCandidateOverride`
...
- `consistency-evidence.ts` — all strength resolvers, atom builders
- `consistency-policy.ts` — evidence contribution scaling
- `case-builder.ts:221-258` — `coverageFromKind`
- `cohort-index.ts` — `resolveCohortEvidenceKind`, signal aggregation
- `util.ts:17-27` — `kindRank`, `mergeEvidenceKind`
...
- `cohort-index.ts` — signal aggregation comparisons
...
- `cohort-index.ts` — text contrast resolution
...
- `cohort-index.ts` — membership resolution
...
- `content-composition.ts:474-498,587-609,618-627` — majority finding, normalization, divergence scoring
...
**NOTE:** `scoring.ts` and `diagnostics.ts` use classification in human-readable messages. Add a lookup table in the file that formats them:
```typescript
...
- `graph.ts:42` — `LayoutElementNode.textualContent`
...
- `cohort-index.ts` — textual content counting
- `case-builder.ts:238-242` — `coverageFromSubjectText`
- `build.ts:386` — dynamic slot candidate check
...
function toScrollAxis(x: boolean, y: boolean): LayoutScrollAxis {
  return ((x ? LayoutScrollAxis.X : 0) | (y ? LayoutScrollAxis.Y : 0)) as LayoutScrollAxis
}
```
